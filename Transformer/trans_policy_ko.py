...
def translate_sentence(sentence, model, opt, s_vocab, t_vocab, s_tokenizer, t_tokenizer):
    
    model.eval()
    indexed = []
    #sentence = SRC.preprocess(sentence)

    sentence = convert_tokens_to_ids(s_vocab, s_tokenizer, sentence)

    sentence = Variable(torch.LongTensor([sentence]))
    sentence = sentence.cuda()
    
    sentence = beam_search(sentence, model, s_vocab, t_vocab, opt)

    #return  multiple_replace({' ?' : '?',' !':'!',' .':'.','\' ':'\'',' ,':','}, sentence)
    
    return sentence

def translate(opt, model, s_vocab, t_vocab, s_tokenizer, t_tokenizer):
    sentences = opt.text.lower().split('.')
    translated = []

    for sentence in sentences:
        result = translate_sentence(sentence + '.', model, opt, s_vocab, t_vocab, s_tokenizer, t_tokenizer)
        

        result = decode_tokens(t_tokenizer, result)
...
def main():
    ...
    if opt.bleu == False:
        ...
        for batch_idx, (text, tgt) in enumerate(zip(src_corpus, tgt_corpus)):
            ...
            phrase = translate(opt, model, en_vocab, ko_vocab, en_sp_tokenizer, ko_sp_tokenizer)
